        ...
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {upcoming.map((event, idx) => (
              <Reveal key={event.id} delay={Math.min(idx * 0.05, 0.3)}>
              <div className="card p-4 flex items-center gap-4 hover:border-accent/30 hover:-translate-y-0.5 transition-all duration-300">
      ...
      {past.length > 0 && (
        <div>
          ...
          <div className="space-y-3 opacity-60">
            {past.map((event, idx) => (
              <Reveal key={event.id} delay={Math.min(idx * 0.04, 0.25)}>
